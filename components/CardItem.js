...
const CardItem = ({ item, navigation, isLoading, onLoading }) => {
  ...
  const [visibleEditBreakpoint, setVisibleEditBreakpoint] = useState(false)
  const showDialogEdit = () => setVisibleEdit(true)
  const hideDialogEdit = () => setVisibleEdit(false)
  const showDialogEditBreakpoint = () => setVisibleEditBreakpoint(true)
  const hideDialogEditBreakpoint = () => setVisibleEditBreakpoint(false)
  const init = async () => {
    let d = []
    for (var i = 1; i <= 100; i++) {
      d.push({
  ...
  const toggleWaterPlant = async (item) => {
    ...
    if (waterPlant) {
      let rs = await axios.post(
        `${REACT_APP_BASE_URL}/plant/control`,
        {
          plantId: item.plantId,
          requestCode: 0,
        },
  ...
  return (
    <Card style={styles.card}>
      ...
      <EditBreakpoint
        ...
        name={item.name}
        image={item.image}
        breakpoint={item.soilMoistureBreakpoint}
        title="Sửa ngưỡng tưới tự động"
        onLoading={onLoading}
        isLoading={isLoading}
        visible={visibleEditBreakpoint}
        onDismiss={hideDialogEditBreakpoint}
      />
      <Card.Content>
        ...
        <View style={{ flexDirection: 'row', marginLeft: -20 }}>
          ...
          >
            ...
            >
              <IconButton icon="water-outline" size={30}></IconButton>
        ...
        >
          ...
          {selected == false ? (
            ...
            >
              <Text
                onPress={() => {
                  showDialogEditBreakpoint()
                }}
              >
                Ngưỡng độ ẩm: {item?.soilMoistureBreakpoint}%
              </Text>
              {/* <Dropdown
